...
if "points" in st.session_state:
    ...
    fig, ax = plt.subplots()
    xs, ys = zip(*points)
    ax.scatter(xs, ys, c="green", marker="o", label="Points")
    ax.scatter(0, 0, c="red", marker="x", label="Point de départ")
    if forme == "Quadrilatère":
        ax.set_xlim(-1, longueur + 1)
        ax.set_ylim(-1, largeur + 1)
        ax.set_title("Quadrilatère")
    else:
        r = diametre / 2
    ...
    if remaining:
        ...
        st.write(f"📏 **Distance** : {distance:.2f} m")
        st.write(f"🧭 **Direction** : {angle_deg:.1f}° (0° = Est, 90° = Nord)")

        if st.button("✅ Marquer comme visité et saisir les espèces"):
            st.session_state["current"] = index
    else:
        st.success("✅ Tous les points ont été visités !")
